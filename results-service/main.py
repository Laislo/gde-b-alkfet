...
class ResultUpdate(BaseModel):
    ...
    def check_assay_value(self):
        if self.assayValue < 0:
            raise ValueError("Az assay érték nem lehet negatív")
        if self.assayValue > 200:
...
@app.patch("/api/results/labid/{lab_id}")
async def update_result_by_labid(lab_id: str, update: ResultUpdate):
    # 1. Keresés Lab ID alapján (még közvetlenül a DB-ből, de már Lab ID-val)
    sample = await db.samples.find_one({"labId": lab_id})
    if not sample:
        raise HTTPException(status_code=404, detail="Nincs meg a minta")

    val = update.assayValue
    # Alapértelmezett értékek, ha hiányoznának a specifikációk
    s_min = sample.get("specMin", 95.0)
    s_max = sample.get("specMax", 105.0)
    ...
    if status == "OOS" and not oos_id:
        oos_count = await db.samples.count_documents({"status": "OOS"})
        oos_id = f"OOS-{now.year}-{(oos_count + 1):03d}"
